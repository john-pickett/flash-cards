    ...
    <div>
      ...
      <div v-if="currentLesson !== null">
            <v-layout row wrap>
                <v-flex xs-6>
                    <v-card>
                        <h4 class="text-center">Title <!-- {{currentLesson.title}} --></h4>

                        <p class="card-text text-center">Spanish Word <!-- {{currentWord}} --></p>
                        <button @click="startLesson" class="md-primary md-raised">Start</button>
                    </v-card>
                </v-flex>
                <v-flex xs-6>
                    <v-card>
...
export default {
    ...
    methods: {
      ...
      evalGuess: function (event) {
        ...
        if (this.currentLesson.answers.indexOf(event.target.value) === this.counter) {
